	...
	for id_sat, d_sat in d.items():
		for t in d_sat.keys():
			if "l1-rho" in d_sat[t] and "l2-rho" in d_sat[t]:
				i_1 = dict_freq["l2"] ** 2 / (dict_freq["l1"] ** 2 - dict_freq["l2"] ** 2) * (
					d_sat[t]["l1-phi"] - d_sat[t]["l2-phi"]
				)
				i_2 = dict_freq["l1"] ** 2 / (dict_freq["l1"] ** 2 - dict_freq["l2"] ** 2) * (
					d_sat[t]["l1-phi"] - d_sat[t]["l2-phi"]
				)
			...
			if "e1-rho" in d_sat[t] and "e5a-rho" in d_sat[t]:
				i_1 = dict_freq["e5a"] ** 2 / (dict_freq["e1"] ** 2 - dict_freq["e5a"] ** 2) * (
					d_sat[t]["e1-phi"] - d_sat[t]["e5a-phi"]
				)
				i_5 = dict_freq["e1"] ** 2 / (dict_freq["e1"] ** 2 - dict_freq["e5a"] ** 2) * (
					d_sat[t]["e1-phi"] - d_sat[t]["e5a-phi"]
				)
...
def create_feature_dataset(df_svstates: pd.DataFrame, df_wls: pd.DataFrame, df_cmc: pd.DataFrame) -> pd.DataFrame:
	...
	for t in tqdm(list_t, total=len(list_t), desc="Feature extraction"):
		sub_data_timestamps = df_svstates[df_svstates["gps_millis"] == t]
		...
		d_cmc_e1.append(
			np.mean(np.abs(sub_data_timestamps_cmc[sub_data_timestamps_cmc["gnss_id"] == "galileo"]["CMC_a"]))
		)
		d_cmc_l2.append(np.mean(np.abs(sub_data_timestamps_cmc[sub_data_timestamps_cmc["gnss_id"] == "gps"]["CMC_b"])))
		d_cmc_e5.append(
			np.mean(np.abs(sub_data_timestamps_cmc[sub_data_timestamps_cmc["gnss_id"] == "galileo"]["CMC_b"]))
		)
...
def process_gnss_feature_extraction(
	...
	df_features = create_feature_dataset(df_svstates, df_wls, cmc_data)
