...
def _local_projection(points_xyz, origin_xyz, u_xy):
    ...
    z_rel = points_xyz[:, 2] - origin_xyz[2]
    dist_h = np.hypot(longitudinal, lateral)
    return longitudinal, lateral, z_rel, dist_h


def _sky_mask_deg(z_rel, dist_h, classes):
...
def _build_slice_view(
    ...
    if z_axis_max is not None:
        z_data_min = float(np.nanmin(z_rel)) if len(z_rel) else -1.0
...
def _build_perspective_view(
    ...
    z_min = float(min(-1.0, np.percentile(z_rel, 1))) if len(z_rel) else -1.0
    z_max = float(z_axis_max) if z_axis_max is not None else float(max(4.0, np.percentile(z_rel, 99)))
...
def render_lidar_slice_2d(trajet_id, lidar_dir, matched_df, gnss_offset_z=0.0):
    ...
    with c1:
        if spatial_mode == "Rayon":
            radius = st.slider("Rayon de masquage (m)", min_value=5, max_value=80, value=20, step=1)
            corridor_width = None
    ...
    longitudinal, lateral, z_rel, dist_h = _local_projection(points_xyz, origin, u_xy)
    # Alignement avec le pipeline d'extraction: decalage du bras de levier ajoute a z_rel.
    z_rel = z_rel + float(gnss_offset_z)
    if spatial_mode == "Couloir":
    ...
    z_default_max = float(np.ceil(max(3.0, np.percentile(z_rel, 99)))) if len(z_rel) else 5.0

    sky = _sky_mask_deg(z_rel, dist_h, classes)
    ...
    if spatial_mode == "Rayon":
        ...
        M[i].metric("Zone", f"Couloir {corridor_width:.1f} x {2.0 * corridor_length:.1f} m")
