...
def _time_to_hms_str(t: datetime.time) -> str:
    return t.strftime("%H:%M:%S")


def _parse_hms_str(value: str):
    txt = str(value).strip()
    try:
...
def render_viz_3d(trajet_id, LIDAR_DIR, matched_df, gnss_offset):
    ...
    time_slider_snapshot_key = f"viz3d_time_slider_snapshot_{trajet_id}"
    time_start_text_key = f"viz3d_time_start_text_{trajet_id}"
    time_end_text_key = f"viz3d_time_end_text_{trajet_id}"
    percent_range_state_key = f"viz3d_percent_range_{trajet_id}"
    percent_slider_snapshot_key = f"viz3d_percent_slider_snapshot_{trajet_id}"
    percent_start_text_key = f"viz3d_percent_start_text_{trajet_id}"
    percent_end_text_key = f"viz3d_percent_end_text_{trajet_id}"
    ...
    with col1:
        ...
        if mode_selection == "Temporel":
            try :
                ...
                # Synchronisation slider -> champs texte
                slider_sig = (_time_to_hms_str(range_val[0]), _time_to_hms_str(range_val[1]))
                last_slider_sig = st.session_state.get(time_slider_snapshot_key)
                if (time_start_text_key not in st.session_state) or (time_end_text_key not in st.session_state):
                    st.session_state[time_start_text_key] = slider_sig[0]
                    st.session_state[time_end_text_key] = slider_sig[1]
                    st.session_state[time_slider_snapshot_key] = slider_sig
                elif slider_sig != last_slider_sig:
                    st.session_state[time_start_text_key] = slider_sig[0]
                    st.session_state[time_end_text_key] = slider_sig[1]
                    st.session_state[time_slider_snapshot_key] = slider_sig
                ...
                with txt_col1:
                    start_txt = st.text_input(
                        "Début (HH:MM:SS)",
                        key=time_start_text_key,
                        help="Saisie manuelle synchronisée avec le slider.",
                    )
                with txt_col2:
                    end_txt = st.text_input(
                        "Fin (HH:MM:SS)",
                        key=time_end_text_key,
                        help="Saisie manuelle synchronisée avec le slider.",
                    )

                start_manual = _parse_hms_str(start_txt)
                end_manual = _parse_hms_str(end_txt)
                if start_manual is None or end_manual is None:
                    valid_inputs = False
                    st.error("Format invalide: utilisez HH:MM:SS pour début/fin.")
                else:
                    if end_manual <= start_manual:
                        valid_inputs = False
                        st.error("L'heure de début doit être strictement inférieure à l'heure de fin.")
                    elif start_manual < min_time or end_manual > max_time:
                        valid_inputs = False
                        st.error("Les heures doivent être dans la plage disponible.")
                    else:
                        manual_sig = (_time_to_hms_str(start_manual), _time_to_hms_str(end_manual))
                        if manual_sig != slider_sig:
                            st.session_state[time_range_state_key] = (start_manual, end_manual)
                            st.session_state[time_slider_snapshot_key] = manual_sig
                            st.rerun()
            ...
            default_pct = st.session_state.get(percent_range_state_key, (0.0, 100.0))
            range_val = st.slider("Plage du trajet (%)", 0.0, 100.0, [float(default_pct[0]), float(default_pct[1])] , step=0.5)
            st.session_state[percent_range_state_key] = (float(range_val[0]), float(range_val[1]))

            # Synchronisation slider -> champs texte
            pct_sig = (f"{float(range_val[0]):.1f}", f"{float(range_val[1]):.1f}")
            last_pct_sig = st.session_state.get(percent_slider_snapshot_key)
            if (percent_start_text_key not in st.session_state) or (percent_end_text_key not in st.session_state):
                st.session_state[percent_start_text_key] = pct_sig[0]
                st.session_state[percent_end_text_key] = pct_sig[1]
                st.session_state[percent_slider_snapshot_key] = pct_sig
            elif pct_sig != last_pct_sig:
                st.session_state[percent_start_text_key] = pct_sig[0]
                st.session_state[percent_end_text_key] = pct_sig[1]
                st.session_state[percent_slider_snapshot_key] = pct_sig
            ...
            with pct_col1:
                pct_start_txt = st.text_input(
                    "Début (%)",
                    key=percent_start_text_key,
                    help="Saisie manuelle synchronisée avec le slider.",
                )
            with pct_col2:
                pct_end_txt = st.text_input(
                    "Fin (%)",
                    key=percent_end_text_key,
                    help="Saisie manuelle synchronisée avec le slider.",
                )

            try:
                pct_start_manual = float(str(pct_start_txt).replace(",", "."))
                pct_end_manual = float(str(pct_end_txt).replace(",", "."))
            except Exception:
                valid_inputs = False
                st.error("Format invalide: entrez des nombres pour les pourcentages.")
                pct_start_manual = None
                pct_end_manual = None

            if pct_start_manual is not None and pct_end_manual is not None:
                if pct_end_manual <= pct_start_manual:
                    valid_inputs = False
                    st.error("Le pourcentage de début doit être strictement inférieur à la fin.")
                elif pct_start_manual < 0.0 or pct_end_manual > 100.0:
                    valid_inputs = False
                    st.error("Les pourcentages doivent être dans [0, 100].")
                else:
                    manual_pct_sig = (f"{pct_start_manual:.1f}", f"{pct_end_manual:.1f}")
                    if manual_pct_sig != pct_sig:
                        st.session_state[percent_range_state_key] = (pct_start_manual, pct_end_manual)
                        st.session_state[percent_slider_snapshot_key] = manual_pct_sig
                        st.rerun()
